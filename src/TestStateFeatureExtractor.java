...
public class TestStateFeatureExtractor {
	
	private int NUM_FEATURES;
	private TestState s;
	private int originalRowsCleared;

	public TestStateFeatureExtractor(TestState s, 
			int numFeatures,
			int originalRowsCleared) {
		this.s = s;
		this.NUM_FEATURES = numFeatures;
		this.originalRowsCleared = originalRowsCleared;
	}
	...
	private double getFeatureFullRows() {
		// the number of full rows is actually the difference 
		// between the number of cleared rows between the 
		// original and the new state
		return s.getRowsCleared() - originalRowsCleared;
	}
	...
	public boolean isHole(TestState s, int[][] field, int i, int j) {
		int count = 0;
